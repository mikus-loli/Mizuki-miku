...
(function () {
	if (window.__pioSdk4Initialized) {
		return;
	}
	window.__pioSdk4Initialized = true;

	var app;
	var currentModel = null;
	var pioModelLoaded = false;

	window.__pioGetAppState = function () {
		return app;
	};
	window.__pioGetModel = function () {
		return currentModel;
	};
	window.__pioIsModelLoaded = function () {
		return pioModelLoaded;
	};

	window.loadlive2d = function (canvas_id, json_object_or_url) {
		if (pioModelLoaded && currentModel) {
			console.log("[Pio] Model already loaded, skipping...");
			return;
		}
		...
		if (!canvas) {
			console.warn("[Pio] Canvas not found");
			return;
		}
	...
	window.initPioPixi = function (alignmentParam) {
		...
		if (app && app.destroyed !== true) {
			console.log("[Pio] Pixi App already exists, refreshing style...");
			pio_refresh_style();
			return;
		}
		...
		if (!canvas) {
			console.warn("[Pio] Canvas element #pio not found");
			return;
		}
		...
		pio_refresh_style();
		console.log(
			"[Pio] Pixi App Initialized with alignment:",
			pio_alignment,
		);
	};
